...
import yaml
from pywebio.output import popup, put_html, put_link, put_markdown, put_text
...
def ios_pop_window():
    with popup(t("iOS快捷指令", "iOS Shortcut")):
        ...
        put_markdown(
            t(
                "快捷指令运行在iOS平台，本快捷指令可以快速调用本项目的公共API将抖音或TikTok的视频或图集下载到你的手机相册中，暂时只支持单个链接进行下载。",
        ...
        put_markdown(
            t(
                "如遇到通知询问是否允许快捷指令访问xxxx (域名或服务器)，需要点击允许才可以正常使用。",
        ...
        put_markdown(
            t(
                "在抖音或TikTok的视频下方点击分享，然后点击复制链接，然后去快捷指令APP中运行该快捷指令。",
        ...
        put_markdown(
            t(
                "如果弹窗询问是否允许读取剪切板请同意，随后快捷指令将链接内容保存至相册中。",
