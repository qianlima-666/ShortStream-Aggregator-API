...
def error_do(reason: str, value: str) -> None:
    ...
    put_table([[ViewsUtils.t("原因", "reason"), ViewsUtils.t("输入值", "input value")], [reason, value]])
    put_markdown(ViewsUtils.t("> 可能的原因:", "> Possible reasons:"))
    put_markdown(
        ViewsUtils.t("- 视频已被删除或者链接不正确。", "- The video has been deleted or the link is incorrect.")
    )
    (put_markdown(ViewsUtils.t("- 接口风控，请求过于频繁。", "- Interface risk control, request too frequent.")),)
    put_markdown(
        ViewsUtils.t(
            "- 没有使用有效的Cookie，如果你部署后没有替换相应的Cookie，可能会导致解析失败。",
    ...
    put_markdown(
        ViewsUtils.t(
            "- 请联系部署者或站点维护者以获取支持。", "- Please contact the deployer or site maintainer for support."
        )
    )
    put_html("<hr>")
...
def parse_video():
    ...
    input_data = textarea(
        ViewsUtils.t(
            "请将抖音或TikTok的分享口令或网址粘贴于此", "Please paste the share code or URL of [Douyin|TikTok] here"
        ),
        type=TEXT,
    ...
    for url in url_lists:
        ...
        if url_type == ViewsUtils.t("视频", "Video"):
            ...
            if wm_video_url_HQ and nwm_video_url_HQ:
                table_list.insert(
                    4,
                    [
                        ViewsUtils.t("视频链接-水印", "Video URL-Watermark"),
                        put_link(ViewsUtils.t("点击查看", "Click to view"), wm_video_url_HQ, new_window=True),
                    ],
                )
                table_list.insert(
                    5,
                    [
                        ViewsUtils.t("视频链接-无水印", "Video URL-No Watermark"),
                        put_link(ViewsUtils.t("点击查看", "Click to view"), nwm_video_url_HQ, new_window=True),
                    ],
                )
            table_list.insert(
                6,
                [
                    ViewsUtils.t("视频下载-水印", "Video Download-Watermark"),
                    put_link(
                        ViewsUtils.t("点击下载", "Click to download"),
            ...
            table_list.insert(
                7,
                [
                    ViewsUtils.t("视频下载-无水印", "Video Download-No-Watermark"),
                    put_link(
                        ViewsUtils.t("点击下载", "Click to download"),
            ...
            # 添加视频信息
            table_list.insert(
                0, [put_video(data.get("video_data").get("nwm_video_url_HQ"), poster=None, loop=True, width="50%")]
            )
        # 如果是图片/If it's image
            ...
            table_list.insert(
                4,
                [
                    ViewsUtils.t("图片打包下载-水印", "Download images ZIP-Watermark"),
                    put_link(
                        ViewsUtils.t("点击下载", "Click to download"),
                        f"/api/download?url={url}&prefix=true&with_watermark=true",
                        new_window=True,
            ...
            table_list.insert(
                5,
                [
                    ViewsUtils.t("图片打包下载-无水印", "Download images ZIP-No-Watermark"),
                    put_link(
                        ViewsUtils.t("点击下载", "Click to download"),
                        f"/api/download?url={url}&prefix=true&with_watermark=false",
                        new_window=True,
            ...
            for image in no_watermark_image_list:
                ...
                table_list.append(
                    [
                        ViewsUtils.t("图片直链: ", "Image URL:"),
                        put_link(ViewsUtils.t("⬆️点击打开图片⬆️", "⬆️Click to open image⬆️"), image, new_window=True),
                    ]
                )
        ...
        with use_scope(str(url_index)):
            ...
            put_info(
                ViewsUtils.t(
                    f"正在解析第{url_index}/{url_count}个链接: ", f"Parsing the {url_index}/{url_count}th link: "
                ),
                put_link(url, url, new_window=True),
                closable=True,
            )
    ...
    with use_scope("result"):
        ...
        # 放置一个按钮，点击后跳转到顶部
        put_button(
            ViewsUtils.t("回到顶部", "Back to top"), onclick=lambda: scroll_to("1"), color="success", outline=True
        )
        # 返回主页链接
