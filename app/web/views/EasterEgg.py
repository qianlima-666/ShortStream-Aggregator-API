...
from pywebio.output import clear, put_html, put_text
...
def a():
    ...
    def h(g):
        return (
            '<table id="life-grid" style="table-layout: fixed; border-spacing:0;">'
            + "".join(
                "<tr>"
                + "".join(
                    f'<td style="width:10px; height:10px; background:{"black" if c else "white"};"></td>' for c in r
                )
                + "</tr>"
                for r in g
            )
