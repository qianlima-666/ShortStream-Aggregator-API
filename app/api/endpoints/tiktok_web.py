...
async def fetch_one_video(
    request: Request, itemId: str = Query(example="7339393672959757570", description="作品id/Video id")
):
    """
    # [中文]
    ### 用途:
...
# 获取用户的个人信息
@router.get("/fetch_user_profile", response_model=ResponseModel, summary="获取用户的个人信息/Get user profile")
...
# 获取用户的作品列表
@router.get("/fetch_user_post", response_model=ResponseModel, summary="获取用户的作品列表/Get user posts")
async def fetch_user_post(
    request: Request,
    secUid: str = Query(
        example="MS4wLjABAAAAv7iSuuXDJGDvJkmH_vz1qkDZYo1apxgzaxdBSeIuPiM", description="用户secUid/User secUid"
    ),
...
# 获取用户的点赞列表
@router.get("/fetch_user_like", response_model=ResponseModel, summary="获取用户的点赞列表/Get user likes")
...
# 获取用户的收藏列表
@router.get("/fetch_user_collect", response_model=ResponseModel, summary="获取用户的收藏列表/Get user favorites")
...
# 获取用户的播放列表
@router.get("/fetch_user_play_list", response_model=ResponseModel, summary="获取用户的播放列表/Get user play list")
...
# 获取用户的合辑列表
@router.get("/fetch_user_mix", response_model=ResponseModel, summary="获取用户的合辑列表/Get user mix list")
async def fetch_user_mix(
    request: Request,
    mixId: str = Query(example="7101538765474106158", description="合辑id/Mix id"),
    cursor: int = Query(default=0, description="翻页游标/Page cursor"),
    count: int = Query(default=30, description="每页数量/Number per page"),
):
    """
...
# 获取作品的评论列表
@router.get("/fetch_post_comment", response_model=ResponseModel, summary="获取作品的评论列表/Get video comments")
...
async def fetch_post_comment_reply(
    ...
    comment_id: str = Query(example="7304877760886588191", description="评论id/Comment id"),
    cursor: int = Query(default=0, description="翻页游标/Page cursor"),
    count: int = Query(default=20, description="每页数量/Number per page"),
    current_region: str = Query(default="", description="当前地区/Current region"),
):
...
# 获取用户的粉丝列表
@router.get("/fetch_user_fans", response_model=ResponseModel, summary="获取用户的粉丝列表/Get user followers")
...
# 获取用户的关注列表
@router.get("/fetch_user_follow", response_model=ResponseModel, summary="获取用户的关注列表/Get user followings")
...
async def generate_ttwid(
    request: Request, cookie: str = Query(example="Your_Cookie", description="用户cookie/User cookie")
):
    """
    # [中文]
    ### 用途:
...
# 生成xbogus
@router.get("/generate_xbogus", response_model=ResponseModel, summary="生成xbogus/Generate xbogus")
...
# 提取列表用户id
@router.get("/get_sec_user_id", response_model=ResponseModel, summary="提取列表用户id/Extract list user id")
async def get_sec_user_id(
    request: Request,
    url: str = Query(example="https://www.tiktok.com/@tiktok", description="用户主页链接/User homepage link"),
):
    """
    # [中文]
    ### 用途:
...
# 提取列表用户id
@router.post("/get_all_sec_user_id", response_model=ResponseModel, summary="提取列表用户id/Extract list user id")
async def get_all_sec_user_id(
    request: Request,
    url: List[str] = Body(example=["https://www.tiktok.com/@tiktok"], description="用户主页链接/User homepage link"),
):
    """
    # [中文]
    ### 用途:
...
async def get_aweme_id(
    request: Request,
    url: str = Query(
        example="https://www.tiktok.com/@owlcitymusic/video/7218694761253735723", description="作品链接/Video link"
    ),
):
    """
    # [中文]
...
# 提取列表作品id
@router.post("/get_all_aweme_id", response_model=ResponseModel, summary="提取列表作品id/Extract list video id")
...
# 获取用户unique_id
@router.get("/get_unique_id", response_model=ResponseModel, summary="获取用户unique_id/Get user unique_id")
...
# 获取列表unique_id列表
@router.post("/get_all_unique_id", response_model=ResponseModel, summary="获取列表unique_id/Get list unique_id")
