...
async def hybrid_parsing_single_video(
    request: Request, url: str = Query(example="https://v.douyin.com/L4FJNR3/"), minimal: bool = Query(default=False)
):
    """
    # [中文]
    ### 用途:
...
# 更新Cookie
@router.post("/update_cookie", response_model=ResponseModel, summary="更新Cookie/Update Cookie")
async def update_cookie_api(
    request: Request,
    ...
    try:
        if service == "douyin":
            ...
            return ResponseModel(
                code=200, router=request.url.path, data={"message": f"Cookie for {service} updated successfully"}
            )
        elif service == "tiktok":
