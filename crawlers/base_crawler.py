...
class BaseCrawler:
    ...
    def __init__(
        ...
        self.limits = httpx.Limits(
            max_connections=max_connections, max_keepalive_connections=max(1, max_connections // 2)
        )
    ...
    def parse_json(self, response: Response) -> dict:
        ...
        Returns:
            dict: 解析后的JSON数据 (Parsed JSON data)
        """
        if response is not None and isinstance(response, Response) and response.status_code == 200:
            try:
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                if not response.text.strip() or not response.content:
                    error_message = "第 {0} 次响应内容为空, 状态码: {1}, URL:{2}".format(
    ...
    def handle_http_status_error(self, http_error, url: str, attempt):
        ...
        if response is None or status_code is None:
            logger.error("HTTP状态错误: {0}, URL: {1}, 尝试次数: {2}".format(http_error, url, attempt))
        ...
        if status_code == 302:
            ...
            logger.error("HTTP状态错误: {0}, URL: {1}, 尝试次数: {2}".format(status_code, url, attempt))
